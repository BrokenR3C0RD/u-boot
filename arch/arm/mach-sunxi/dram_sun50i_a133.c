...
static void mctl_set_addrmap(const struct dram_config *config)
{
	...
	writel_relaxed(ADDRMAP5_ROW0_B6(hif_offset) | ADDRMAP5_ROW1_B7(hif_offset + 1) |
	...
	if (rank_bits == 0)
		...
		writel_relaxed(ADDRMAP0_CS0_B6(hif_offset), &mctl_ctl->addrmap[0]);
}
