...
void mctl_set_timing_params(const struct dram_para *para)
{
	...
	writel_relaxed(tcke | tckesr << 8 | tcksre << 16 | tcksrx << 24,
		       &mctl_ctl->dramtmg[5]);
	writel_relaxed((txp + 2) | 0x20 << 16 | 0x20 << 24,
		       &mctl_ctl->dramtmg[6]);
	writel_relaxed(txs_x32 | 0x10 << 8 | txs_abort_x32 << 16 |
			       txs_abort_x32 << 24,
		       &mctl_ctl->dramtmg[8]);
	writel_relaxed(wr2rd_s | trrd_s << 8 | 0x2 << 16,
		       &mctl_ctl->dramtmg[9]);
	...
	writel_relaxed(para->mr0 << 16 | para->mr1, &mctl_ctl->init[3]);
	writel_relaxed(para->mr2 << 16 | para->mr3, &mctl_ctl->init[4]);
	writel_relaxed(para->mr4 << 16 | para->mr5, &mctl_ctl->init[6]);
	writel_relaxed(para->mr6, &mctl_ctl->init[7]);

	clrsetbits_le32(&mctl_ctl->rankctl, 0xff0, 0x660);
	writel_relaxed((dfi_tphy_wrlat - 1) | 0x2000000 |
			       (dfi_trddata_en - 1) << 16 | 0x808000,
		       &mctl_ctl->dfitmg0);
